...
`;let Mt=class extends ct{constructor(){super(...arguments),this.items=[]}render(){return q` <div class="sortable-list">${this.items.map(t=>this.renderItem(t))}</div> `}renderItem(t){const e=this._draggedId===t.id,i=this._dropTarget?.id===t.id&&"top"===this._dropTarget.position,s=this._dropTarget?.id===t.id&&"bottom"===this._dropTarget.position;return q`
    ...
    `}_renderSubjects(t){const e=this._attrs;if(!e)return q`<div class="section-note">${t("editor.no_entity")}</div>`;const i=jt(e,this._config),s=i.map(t=>({id:t.subject_id,label:t.name}));return q`
    ...
    `}};e([ut({attribute:!1})],zt.prototype,"hass",void 0),e([gt()],zt.prototype,"_config",void 0),zt=e([lt("skolaonline-znamky-ui-marks-all-editor")],zt);const Ot="skolaonline-znamky-ui-marks-all-card";let Nt=class extends ct{constructor(){super(...arguments),this._historyLoading=!1,this._newMarkIds=new Set}setConfig(t){if(!t?.entity)throw new Error('skolaonline-znamky-ui-marks-all-card: "entity" is required');this._config=t}getCardSize(){return 2+Math.max(1,this._subjects.length)}static getConfigElement(){return document.createElement("skolaonline-znamky-ui-marks-all-editor")}static getStubConfig(t,e,i){const s=t,r=[...e,...i].find(t=>"skolaonline_znamky"===s.entities?.[t]?.platform);if(!r)throw new Error("No skolaonline_znamky entity available");return{type:`custom:${Ot}`,entity:r}}connectedCallback(){super.connectedCallback(),this._subscribeNewMarks()}disconnectedCallback(){super.disconnectedCallback(),this._unsubscribeEvents?.(),this._unsubscribeEvents=void 0}updated(t){t.has("hass")&&!this._unsubscribeEvents&&this._subscribeNewMarks()}async _subscribeNewMarks(){if(this.hass&&!this._unsubscribeEvents)try{this._unsubscribeEvents=await this.hass.connection.subscribeEvents(t=>this._onNewMarkEvent(t.data),"skolaonline_znamky_new_mark")}catch(t){console.error(`${Ot}: failed to subscribe to skolaonline_znamky_new_mark`,t)}}_onNewMarkEvent(t){t.student_id===this._attrs?.student_id&&(this._newMarkIds=new Set(this._newMarkIds).add(t.mark_id))}get _attrs(){if(this.hass&&this._config)return this.hass.states[this._config.entity]?.attributes}get _subjects(){const t=this._attrs;return t?jt(t,this._config??{}):[]}render(){if(!this._config)return q``;const t=wt(this.hass),e=this._attrs;if(!e||!this.hass)return q`<ha-card><div class="empty">${t("card.no_entity")}</div></ha-card>`;const i=Number(this.hass.states[this._config.entity].state);return q`
      <ha-card>
        <div class="header">
          <div>
        ...
        ${void 0!==this._historyTarget?this._renderHistory(t):F}
      </ha-card>
    `}_renderSubjectRow(t){const e=[...t.marks].sort((t,e)=>t.date<e.date?1:-1),i=t.count-e.length;return q`
      <div class="subject-row" style="--subject-color:${t.color??"var(--primary-color)"}" @click=${()=>this._toggleHistory(t.subject_id)}>
        ...
        <div class="marks">${e.map(t=>this._renderMarkChip(t))} ${i>0?q`<div class="more-marks">+${i}</div>`:F}</div>
      </div>
    `}_renderMarkChip(t){const e=At(t.value),i=null===e?"var(--disabled-text-color, #9e9e9e)":Et(e),s=Ct(t.id,t.date,this._newMarkIds),r=t.weight>.2;return q`
      <div class="mark-chip ${r?"heavy":""} ${s?"new":""}" style="background:${i}" title="${t.date.slice(0,10)} · ${t.weight}">${t.value}</div>
    `}async _toggleHistory(t){if(this._historyTarget===t)return void(this._historyTarget=void 0);this._historyTarget=t,this._historyMarks=void 0,this._historyError=void 0;const e=this._attrs;if(this.hass&&this._config&&e){this._historyLoading=!0;try{this._historyMarks=await St(this.hass,this._config.entity,e.student_id,t??void 0)}catch(t){this._historyError=t instanceof Error?t.message:String(t)}finally{this._historyLoading=!1}}}_renderHistory(t){if(this._historyLoading)return q`<div class="history">
        <div class="history-loading"><ha-spinner size="small"></ha-spinner></div>
      </div>`;if(this._historyError)return q`<div class="history"><ha-alert alert-type="error">${this._historyError}</ha-alert></div>`;if(!this._historyMarks?.length)return q`<div class="history"><div class="empty">${t("card.history_empty")}</div></div>`;const e=[...this._historyMarks].sort((t,e)=>t.date<e.date?1:-1);return q` <div class="history">${e.map(t=>this._renderHistoryRow(t))}</div> `}_renderHistoryRow(t){const e=At(t.value),i=null===e?"inherit":Et(e);return q`
    ...
    `}static get styles(){return Tt}};e([ut({attribute:!1})],Nt.prototype,"hass",void 0),e([gt()],Nt.prototype,"_config",void 0),e([gt()],Nt.prototype,"_historyTarget",void 0),e([gt()],Nt.prototype,"_historyMarks",void 0),e([gt()],Nt.prototype,"_historyLoading",void 0),e([gt()],Nt.prototype,"_historyError",void 0),e([gt()],Nt.prototype,"_newMarkIds",void 0),Nt=e([lt(Ot)],Nt),window.customCards=window.customCards||[],window.customCards.push({type:Ot,name:"Škola OnLine – Známky",description:"Karta se známkami dítěte ze zálohové integrace skolaonline_znamky.",preview:!0,documentationURL:"https://github.com/hondzik/skolaonline-znamky-ui"}),function(){const e="padding: 2px 4px; font-family: Roboto,Verdana,Geneva,sans-serif;",i=`background-color: rgb(255, 127, 15); color: rgb(0, 0, 49); ${e}`,s=`background-color: rgb(0, 0, 49); color: rgb(255, 127, 15); ${e}`;console.groupCollapsed("%cLovelace Cards for skola-online-znamky integration%c0.0.1",i,s),console.info("Lovelace Cards for skola-online-znamky integration"),console.info(`Github: ${t}`),console.groupEnd()}();
