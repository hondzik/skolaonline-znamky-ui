...
`;let Ot=class extends dt{constructor(){super(...arguments),this.items=[]}render(){return W` <div class="sortable-list">${this.items.map(t=>this.renderItem(t))}</div> `}renderItem(t){const e=this._draggedId===t.id,i=this._dropTarget?.id===t.id&&"top"===this._dropTarget.position,s=this._dropTarget?.id===t.id&&"bottom"===this._dropTarget.position;return W`
    ...
    `}_renderSubjects(t){const e=this._attrs;if(!e)return W`<div class="section-note">${t("editor.no_entity")}</div>`;const i=Tt(e,this._config),s=i.map(t=>({id:t.subject_id,label:t.name,dimmed:t.hidden}));return W`
    ...
    `}_renderSubjectRow(t,e){const i=[...t.marks].sort((t,e)=>t.date<e.date?1:-1),s=t.count-i.length,r=this._referenceWeight,o=this._weightRange,n=this._config?.border_width??8,a=()=>this._toggleHistory(t.subject_id);return W`
      <div class="subject-row" style="background:${t.color??"var(--primary-color)"};padding-left:${n}px">
        <div class="subject-row-inner">
          ...
          <div class="marks" @click=${a}>
            ${i.length?W`${i.map(t=>this._renderMarkChip(t,r,o))} ${s>0?W`<div class="more-marks">+${s}</div>`:q}`:W`<div class="no-marks">${e("card.no_marks")}</div>`}
          </div>
          ${this._expandedSubjectId===t.subject_id?this._renderMarksList(this._allMarks.filter(e=>e.subject_id===t.subject_id),e):q}
        </div>
      </div>
    `}_renderMarkChip(t,e,i){const s=At(t.value),r=null===s,o=r?"var(--disabled-text-color, #9e9e9e)":jt(s),n=function(t,e,i,s=1,r=new Date){return!(s<=0)&&(i.has(t)||Mt(e,r,s))}(t.id,t.date,this._newMarkIds,this._config?.new_mark_days??1),a=e>0&&!r&&t.weight>e,d=r?(i.min+i.max)/2:t.weight,c=this._config?.size_by_weight?`width:${zt(d,i.min,i.max)}em;height:${zt(d,i.min,i.max)}em;`:"";return W`
