...
`;let Ot=class extends dt{constructor(){super(...arguments),this.items=[]}render(){return V` <div class="sortable-list">${this.items.map(t=>this.renderItem(t))}</div> `}renderItem(t){const e=this._draggedId===t.id,i=this._dropTarget?.id===t.id&&"top"===this._dropTarget.position,s=this._dropTarget?.id===t.id&&"bottom"===this._dropTarget.position;return V`
    ...
    `}onDragStart(t,e){this._draggedId=e.id,t.dataTransfer&&(t.dataTransfer.effectAllowed="move")}onDragEnd(){this._draggedId=void 0,this._dropTarget=void 0}onDragOver(t,e){if(t.preventDefault(),t.dataTransfer&&(t.dataTransfer.dropEffect="move"),e.id===this._draggedId)return;const i=t.currentTarget.getBoundingClientRect(),s=i.top+i.height/2;this._dropTarget={id:e.id,position:t.clientY<s?"top":"bottom"}}onDragLeave(t,e){this._dropTarget?.id===e.id&&t.target===t.currentTarget&&(this._dropTarget=void 0)}onDrop(t,e){t.preventDefault(),t.stopPropagation();const i=this._draggedId,s=this._dropTarget?.position;if(this._dropTarget=void 0,!i||i===e.id)return;const r=[...this.items],o=r.findIndex(t=>t.id===i);if(-1===o)return;const[n]=r.splice(o,1);let a=r.findIndex(t=>t.id===e.id);"bottom"===s&&(a+=1),r.splice(a,0,n),this.items=r,this.dispatchEvent(new CustomEvent("reorder",{detail:{items:r},bubbles:!0,composed:!0}))}static get styles(){return Nt}};e([pt({attribute:!1})],Ot.prototype,"items",void 0),e([pt({attribute:!1})],Ot.prototype,"renderLeading",void 0),e([pt({attribute:!1})],Ot.prototype,"renderTrailing",void 0),e([_t()],Ot.prototype,"_draggedId",void 0),e([_t()],Ot.prototype,"_dropTarget",void 0),Ot=e([ht("so-sortable-list")],Ot);let Rt=class extends dt{constructor(){super(...arguments),this._config={type:"custom:skolaonline-znamky-ui-marks-all-card",entity:""}}setConfig(t){this._config={...t}}get _attrs(){if(this.hass&&this._config.entity)return this.hass.states[this._config.entity]?.attributes}render(){if(!this.hass)return V``;const t=kt(this.hass);return V`
      <div class="card-config">
        ...
        <ha-selector
          .hass=${this.hass}
          .selector=${{number:{min:0,max:30,step:1,mode:"slider",unit_of_measurement:t("editor.new_mark_days_unit")}}}
          .value=${this._config.new_mark_days??1}
          .label=${t("editor.new_mark_days")}
          @value-changed=${this._newMarkDaysChanged}
        ></ha-selector>
        <div class="section-description">${t("editor.new_mark_days_description")}</div>

        ${this._renderSubjects(t)}
      </div>
    `}_renderSubjects(t){const e=this._attrs;if(!e)return V`<div class="section-note">${t("editor.no_entity")}</div>`;const i=Tt(e,this._config),s=i.map(t=>({id:t.subject_id,label:t.name,dimmed:t.hidden}));return V`
      ...
      ></so-sortable-list>
    `}_renderColorSwatch(t,e){const i=e.find(e=>e.subject_id===t.id)?.color??"#3f51b5";return V`<input type="color" class="color-swatch" .value=${i} @click=${t=>t.stopPropagation()} @input=${e=>this._colorChanged(t.id,e)} />`}_renderRowActions(t,e,i){const s=e.find(e=>e.subject_id===t.id)?.hidden??!1;return V`
    ...
    `}};e([pt({attribute:!1})],Rt.prototype,"hass",void 0),e([_t()],Rt.prototype,"_config",void 0),Rt=e([ht("skolaonline-znamky-ui-marks-all-editor")],Rt);const Dt="skolaonline-znamky-ui-marks-all-card";let Ut=class extends dt{constructor(){super(...arguments),this._historyLoading=!1,this._newMarkIds=new Set,this._refreshing=!1}setConfig(t){if(!t?.entity)throw new Error('skolaonline-znamky-ui-marks-all-card: "entity" is required');this._config=t}getCardSize(){return 2+Math.max(1,this._visibleSubjects.length)}static getConfigElement(){return document.createElement("skolaonline-znamky-ui-marks-all-editor")}static getStubConfig(t,e,i){const s=t,r=[...e,...i].find(t=>"skolaonline_znamky"===s.entities?.[t]?.platform);if(!r)throw new Error("No skolaonline_znamky entity available");return{type:`custom:${Dt}`,entity:r}}connectedCallback(){super.connectedCallback(),this._subscribeNewMarks()}disconnectedCallback(){super.disconnectedCallback(),this._unsubscribeEvents?.(),this._unsubscribeEvents=void 0}updated(t){t.has("hass")&&!this._unsubscribeEvents&&this._subscribeNewMarks()}async _subscribeNewMarks(){if(this.hass&&!this._unsubscribeEvents)try{this._unsubscribeEvents=await this.hass.connection.subscribeEvents(t=>this._onNewMarkEvent(t.data),"skolaonline_znamky_new_mark")}catch(t){console.error(`${Dt}: failed to subscribe to skolaonline_znamky_new_mark`,t)}}_onNewMarkEvent(t){t.student_id===this._attrs?.student_id&&(this._newMarkIds=new Set(this._newMarkIds).add(t.mark_id),this._invalidateMarksCache())}get _attrs(){if(this.hass&&this._config)return this.hass.states[this._config.entity]?.attributes}get _subjects(){const t=this._attrs;return t?Tt(t,this._config??{}):[]}get _visibleSubjects(){const t=this._config?.show_empty_subjects??!0;return this._subjects.filter(e=>!e.hidden&&(t||e.count>0))}get _referenceWeight(){const t=this._attrs;return t?0===(e=t.subjects.flatMap(t=>t.marks.map(t=>t.weight))).length?0:e.reduce((t,e)=>t+e,0)/e.length:0;var e}get _weightRange(){const t=this._attrs;return t?0===(e=t.subjects.flatMap(t=>t.marks.map(t=>t.weight))).length?{min:0,max:0}:{min:Math.min(...e),max:Math.max(...e)}:{min:0,max:0};var e}get _allMarks(){return this._marksCache?.marks??[]}_invalidateMarksCache(){this._marksCache=void 0,void 0!==this._expandedSubjectId&&this._ensureMarksLoaded()}render(){if(!this._config)return V``;const t=kt(this.hass),e=this._attrs;if(!e||!this.hass)return V`<ha-card><div class="empty">${t("card.no_entity")}</div></ha-card>`;const i=Number(this.hass.states[this._config.entity].state),s=`--soz-title-font-size:${this._config.title_font_size??20}px;--soz-subject-font-size:${this._config.subject_font_size??15}px;--soz-marks-font-size:${this._config.marks_font_size??14}px;`;return V`
      <ha-card style=${s}>
        <div class="header">
          <div>
          ...
          <div class="header-right">
            <ha-icon-button class="refresh-button" .label=${t("card.refresh")} .disabled=${this._refreshing} @click=${()=>this._refresh()}>
              ${this._refreshing?V`<ha-spinner size="small"></ha-spinner>`:V`<ha-icon icon="mdi:reload"></ha-icon>`}
            </ha-icon-button>
    ...
    `}_renderSubjectRow(t,e){const i=[...t.marks].sort((t,e)=>t.date<e.date?1:-1),s=t.count-i.length,r=this._referenceWeight,o=this._weightRange,n=this._config?.border_width??8,a=()=>this._toggleHistory(t.subject_id);return V`
      <div class="subject-row" style="background:${t.color??"var(--primary-color)"};padding-left:${n}px">
        <div class="subject-row-inner">
          <div class="subject-row-top" @click=${a}>
          ...
          <div class="marks" @click=${a}>
            ${i.length?V`${i.map(t=>this._renderMarkChip(t,r,o))} ${s>0?V`<div class="more-marks">+${s}</div>`:q}`:V`<div class="no-marks">${e("card.no_marks")}</div>`}
          </div>
          ${this._expandedSubjectId===t.subject_id?this._renderMarksList(this._allMarks.filter(e=>e.subject_id===t.subject_id),e):q}
        </div>
      </div>
    `}_renderMarkChip(t,e,i){const s=At(t.value),r=null===s,o=r?"var(--disabled-text-color, #9e9e9e)":jt(s),n=function(t,e,i,s=1,r=new Date){return!(s<=0)&&(i.has(t)||Mt(e,r,s))}(t.id,t.date,this._newMarkIds,this._config?.new_mark_days??1),a=e>0&&!r&&t.weight>e,d=r?(i.min+i.max)/2:t.weight,c=this._config?.size_by_weight?`width:${zt(d,i.min,i.max)}em;height:${zt(d,i.min,i.max)}em;`:"";return V`
      <div class="mark-chip ${a?"heavy":""} ${n?"new":""}" style="background:${o};${c}" title="${t.date.slice(0,10)} · ${t.weight}">
        ${t.value}
      </div>
    `}async _refresh(){if(this.hass&&this._config&&!this._refreshing){this._refreshing=!0,this._invalidateMarksCache();try{await async function(t,e){await t.callService("homeassistant","update_entity",void 0,{entity_id:e})}(this.hass,this._config.entity)}catch(t){console.error(`${Dt}: failed to refresh marks`,t)}finally{this._refreshing=!1}}}async _toggleHistory(t){this._expandedSubjectId!==t?(this._expandedSubjectId=t,await this._ensureMarksLoaded()):this._expandedSubjectId=void 0}async _ensureMarksLoaded(){const t=this._attrs;if(!this.hass||!this._config||!t)return;const e=this._marksCache;if(!(!!e&&e.semesterId===t.semester_id&&Date.now()-e.fetchedAt<18e5)){this._historyLoading=!0,this._historyError=void 0;try{const e=await Ct(this.hass,this._config.entity,t.student_id);this._marksCache={fetchedAt:Date.now(),semesterId:t.semester_id,marks:e}}catch(t){this._historyError=t instanceof Error?t.message:String(t)}finally{this._historyLoading=!1}}}_renderMarksList(t,e){if(this._historyLoading)return V`<div class="history">
        <div class="history-loading"><ha-spinner size="small"></ha-spinner></div>
      </div>`;if(this._historyError)return V`<div class="history"><ha-alert alert-type="error">${this._historyError}</ha-alert></div>`;if(!t.length)return V`<div class="history"><div class="empty">${e("card.history_empty")}</div></div>`;const i=[...t].sort((t,e)=>t.date<e.date?1:-1);return V` <div class="history">${i.map(t=>this._renderHistoryRow(t))}</div> `}_renderHistoryRow(t){const e=At(t.value),i=null===e?"inherit":jt(e);return V`
      <div class="history-row">
        <span class="history-date">${function(t,e){const i=new Date(t);return Number.isNaN(i.getTime())?t.slice(0,10):new Intl.DateTimeFormat(e,{day:"numeric",month:"numeric",year:"numeric"}).format(i)}(t.date,this.hass?.locale.language)}</span>
    ...
    `}static get styles(){return Pt}};e([pt({attribute:!1})],Ut.prototype,"hass",void 0),e([_t()],Ut.prototype,"_config",void 0),e([_t()],Ut.prototype,"_expandedSubjectId",void 0),e([_t()],Ut.prototype,"_historyLoading",void 0),e([_t()],Ut.prototype,"_historyError",void 0),e([_t()],Ut.prototype,"_marksCache",void 0),e([_t()],Ut.prototype,"_newMarkIds",void 0),e([_t()],Ut.prototype,"_refreshing",void 0),Ut=e([ht(Dt)],Ut),window.customCards=window.customCards||[],window.customCards.push({type:Dt,name:"Škola OnLine – Známky",description:"Karta se známkami dítěte ze zálohové integrace skolaonline_znamky.",preview:!0,documentationURL:"https://github.com/hondzik/skolaonline-znamky-ui"}),function(){const e="padding: 2px 4px; font-family: Roboto,Verdana,Geneva,sans-serif;",i=`background-color: rgb(255, 127, 15); color: rgb(0, 0, 49); ${e}`,s=`background-color: rgb(0, 0, 49); color: rgb(255, 127, 15); ${e}`;console.groupCollapsed("%cLovelace Cards for skola-online-znamky integration%c0.2.0",i,s),console.info("Lovelace Cards for skola-online-znamky integration"),console.info(`Github: ${t}`),console.groupEnd()}();
